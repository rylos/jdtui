...
3. `git push origin main --follow-tags`. This fires `.github/workflows/release.yml`: fmt + clippy + tests, then builds x86_64/aarch64 Linux (musl, static), x86_64/aarch64 macOS and x86_64 Windows, creates the release if it does not exist — with the notes read from the tag OBJECT (the checkout peels the tag, so `--notes-from-tag` would fall back to the commit message; that happened for 1.9.0 and was fixed by hand) — and uploads the archives plus SHA256SUMS.
...
5. Publish to crates.io: `cargo publish` (the crate is `jdtui`; the user logs in with `cargo login` in a normal terminal and REVOKES the token afterwards, so expect no token to be present; a broken version is `cargo yank --version X.Y.Z jdtui` plus `gh release delete vX.Y.Z --cleanup-tag`). `.serena` and `.mcp.json` are excluded from the package by `exclude` in Cargo.toml — keep it that way, they are working notes.
6. Install locally: `cargo build --release && cp target/release/jdtui ~/.local/bin/`. Align the other machine (its ssh command is in the user's global CLAUDE.md, not here — this file is published).
