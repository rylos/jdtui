...
5. Publish to crates.io: `cargo publish` (the crate is `jdtui`; a token lives in ~/.cargo/credentials.toml, and the account needs a verified email). `.serena` and `.mcp.json` are excluded from the package by `exclude` in Cargo.toml — keep it that way, they are working notes.
