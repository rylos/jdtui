...
- `model.rs` — view model: `Tab`, `Row`/`RowKey`, `build_rows`, `Action` enum, `context_menu(tab, packages, rows, stop_mark)`, `Form`/`Field` constructors (login, add_links, rename, directory, new_package, archive_password, filter), `Form::set_value`/`active_label`, `PRIORITIES`, `stop_mark_target`/`row_stop_marked`, `resolve_folder`, `device_menu(update_available, captchas)`.
- `api.rs` — `JdApi` device calls; data types `Package`, `Link` (with `variants`/`variant`), `LinkVariant`, `Snapshot`, `Status`, `RemoveMode`, `AddLinks`, `ArchiveStatus`, `CaptchaJob`, `Account`, `Event`, `FolderPolicy`, `StorageInfo`. `status()` = slow part (stop mark, collecting, extraction queue, captchas); `snapshot(status)` = state, speed, downloads, grabber (4 round trips, ~110 ms each via relay).
...
- `config.rs` — `~/.config/jdtui/config.toml`, mode 0600. Keys: email, password, device, refresh_ms, events, direct (bool, default true), direct_addresses (`host:port` list). `Config::direct()` → `Option<Vec<String>>` (None = relay only) = `poller::Direct`.
