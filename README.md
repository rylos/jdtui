...
so in the About panel (`D`, then About) and once in the footer. Opening the panel asks
GitHub there and then, whatever the cache holds, because that is the moment
somebody is reading the answer; everywhere else the once-a-day timer applies,
and the panel says when it last looked. It is the
